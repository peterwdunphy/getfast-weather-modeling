...
\begin{equation}
  ...
  \label{eq:production}
\end{equation}
for the reference runner. Two remarks on $\kappa$. It is conservative: in level
running almost no work is done on the environment beyond overcoming air
resistance, so the mechanical work the convention credits is dissipated inside
the body within the stride cycle and reappears as heat, which means
\eqref{eq:production} understates heat production rather than overstating it.
And whatever residual drift heat imposes on $\kappa$ runs against the runner
rather than for them, since the added ventilatory and cardiac work of
thermoregulating is itself metabolic and therefore itself a heat source.
...
counterpart of the wet-bulb limit discussed for resting humans, and it is lower
for the same reason skin is cooler: running-speed airflow over wet skin holds
...
capacity of blood. For the reference runner holding a core temperature of
...
$35\,^{\circ}$C. Those are lower bounds. They credit the skin with the whole
production, which overstates it by the small respiratory share, but they also
assume blood returns from the skin fully cooled to skin temperature, which
