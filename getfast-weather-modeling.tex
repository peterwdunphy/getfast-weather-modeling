...
What separates this study from the existing weather literature is that its output is a forecast. Work quantifying heat effects is retrospective: it regresses observed finishing times on the conditions those races happened to be run in, at the level of a field rather than a runner, and the best-performing of these models depend on information that exists only once a race is underway. \citet{weiss2024}, for instance, reaches an $R^2$ of 0.79 only after adding each runner's first 5\,km split, against 0.17 from weather alone. Such models explain what happened; they cannot be handed to a runner beforehand. The predictors that do look forward, including those on consumer running platforms, work from training history and take no account of conditions at all. This paper joins the two. It predicts pace for a run that has not yet taken place, from that runner's prior training and the conditions forecast for it, and it is evaluated on runners withheld entirely from fitting. To our knowledge this is the first prospective, individual-level pace prediction to incorporate a heat response, and the first such response learned from training histories rather than from race results.

The population curve that results is well scaled rather than merely correlated with conditions. Its calibration coefficient is compatible with one, meaning the magnitude the deep model learned matches the correction the pace model independently requires, and it falls where the published literature places a cohort running at training rather than racing effort. Its most consequential property is directional. A weather-blind model predicts runs in the hottest conditions faster than they were actually run, and supplying the curve removes that optimism. Because the error such a model makes is systematically in the runner's disfavor on precisely the days when conditions matter most, correcting its direction is worth more than the small reduction in average error suggests. The magnitude is quantifiable: at $28\,^{\circ}$C the curve implies roughly eleven additional minutes for a runner whose cool-weather marathon is 3:30 and twelve for a four-hour runner.

The per-runner result is more equivocal, and it locates the difficulty precisely. A score matched to the correct runner improves prediction reproducibly, and shuffling that match removes the gain, so the score carries genuine individual information. Yet its effect is nearly independent of temperature, and its residual direction inverts once heart rate is conditioned on. Both observations are what would be expected if heat sensitivity were already partly expressed through the physiological signals the baseline observes, in which case the personalized feature absorbs what remains rather than the effect itself. Separating a runner's heat sensitivity from its own downstream consequences therefore requires either a baseline that excludes those consequences or measurements that identify them directly, and the sensitivity analysis reported above shows that the first is not sufficient on its own.
