...
Adding the deep-learning heat information to the weather-blind XGBoost improves held-out accuracy, and a negative control confirms that the personalized part is real (Figure~\ref{fig:mape_ladder}). Robust MAPE falls from 7.3381\% for the weather-blind model to 7.2936\% when the population DL curve is added, followed by another 7.1304\% decrease when the per-runner sensitivity score is added on top. The current-inclusive, prior, and strict-prior versions of the personalized score perform almost identically.

The controls behave as expected. The shuffled control, which reassigns each runner's sensitivity score to a random runner, lands at 7.2996\% (essentially tied with the population model). In runner-clustered bootstrap terms, the personalized gain beyond the population curve is 0.1632 points with a 95\% interval of [0.039, 0.308] while the shuffled control's interval [$-0.051$, 0.034] straddles it. The population curve's own gain over the weather-blind model, 0.0445 points, has an interval of [$-0.009$, 0.101] that narrowly includes zero, so the population feature is a modest and only marginally significant improvement on this cohort.
...
Stratifying the held-out error by WBGT demonstrates the value of adding weather features (Figure~\ref{fig:error_by_temp}). The weather-blind model displays an expected pattern: it predicts too slow in cold and mild conditions (a signed bias near $-2\%$) and swings to too fast in the hottest bin (a bias of $+0.75\%$ in the 22 to 25\,\textdegree C band, meaning it over-predicts pace on hot runs). The population DL feature lowers MAPE across the range, including the hottest bin. The personalized model lowers MAPE in every temperature bin and places the signed bias closest to zero throughout.
...
The deep model's population heat curve can be compared to the published literature without any shared fitting (Figure~\ref{fig:heateffects}). Expressed as a percent slowdown versus a $10\,^{\circ}$C anchor, the published forms fall into two bands: steep slopes for amateur and mass fields (\citealp{ely2007, nikolaidis2019, knechtle2019, vernon2021}, reaching 10 to 16\% by $30\,^{\circ}$C) and shallow slopes for elite fields (\citealp{mantzios2022, martin1999, guy2014}, near 3 to 5\%). Our curve sits at the lower edge of this range, close to the elite and shallow-amateur forms, and reaching about 5\% by $28\,^{\circ}$C, which is the expected place for a mixed-effort cohort whose runs are mostly training rather than racing.
...
The personalized score improves prediction, but three diagnostics indicate that its contribution is largely a runner-level calibration offset rather than a heat-specific interaction, and that its residual direction is reversed. The change the personalized score makes to the prediction is nearly flat across WBGT. Relative to the population model, it shifts the prediction by about $-0.4$ to $-0.6\%$ across temperature bins, with essentially no row-level association with WBGT ($r = -0.027$); the change is instead ordered by the runner's DL-score quintile. This creates the parallel temperature-bin curves in Figure~\ref{fig:error_by_temp}.

Forcing the personalization to act through the temperature curve keeps only part of the gain. When personalization is supplied as a percentile-matched deviation of the DL curve, so that its input is zero at and below the $10\,^{\circ}$C anchor, the model reaches 7.2043\% robust MAPE, a gain of 0.0893 points beyond the population curve (95\% interval [0.015, 0.167], which excludes zero).
