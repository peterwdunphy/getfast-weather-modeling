...
Heat imposes the same thermodynamic accounting on every runner, but not the same constraint: the balance in Equation~1 holds universally while its terms do not, since sweat capacity, body surface area and the mass over which heat is stored all differ between individuals, and two runners in identical conditions can therefore sit on opposite sides of the compensable threshold (Appendix~\ref{app:physics}). The cost heat exacts consequently varies widely. \textit{\textbf{CORRECTED:} the constraint is not identical across runners; sweat rate and body size enter $E_{\max}$ and $E_{\mathrm{req}}$ directly.} Finishing position, body composition, sex, and age each capture part of that variation---each an observable correlates of a latent heat tolerance. This unobserved quality motivates our approach of inferring heat tolerance from a runner's own history of running in varied conditions rather than using demographic proxies.
...
quintiles are close together and the effect appears in the hardest one. This matters for interpretation
...
\caption{The heat penalty as a function of both conditions and effort, estimated within runner. Left:
the slope above WBGT 15 by heart-rate quintile, with runner-clustered 95\% intervals. Right: the
cumulative penalty implied by the directly estimated slopes for the easiest, middle and hardest fifths.
The right panel plots the quintile estimates rather than a fitted interaction surface, because the
linear interaction term is not distinguishable from zero.}
...
above; a three-percent effect is not visible against day-to-day variation in a per-run error statistic,
...
intervals. A correction that has removed the conditions-dependence sits flat near zero across all three
...
The per-runner result is more equivocal. A score matched to the correct runner improves prediction reproducibly, and shuffling that match removes the gain, so the score carries genuine individual information. Its effect is, moreover, clearly temperature-dependent: interacted with the heat hinges, the score carries $+0.13\%$ per \textdegree C above 15\,\textdegree C and a further $+0.65\%$ per \textdegree C above 24\,\textdegree C, so a runner scored as more sensitive requires a progressively slower prediction as conditions worsen. What does not survive is conditioning: the residual direction inverts once heart rate enters the baseline, and it does so in the linear specification as well as in the gradient-boosted calibration, so the reversal is a property of the conditioning set rather than of any one estimator. \textit{\textbf{REVISED:} the previous claim that the effect was nearly independent of temperature was wrong and is replaced by the measured interaction; the reversal is now reported for both specifications.} Both results are what would be expected if heat sensitivity were already partly expressed through the physiological signals the baseline observes, in which case the personalized feature absorbs what remains rather than the effect itself. Supplying the personalized term alone, without the population curve, is consistent with that reading: error is essentially unchanged (7.20\% against 7.19\% for the population curve alone, and 7.22\% with no heat term at all) and the coefficients keep the physiologically expected sign, but the population-level optimism in heat is left uncorrected, which is the population curve's role and not the individual term's. \textit{\textbf{ADDED:} the personalized-only specification, showing similar error and correct direction but no population-level correction.} Separating a runner's heat sensitivity from its own downstream consequences therefore requires either a baseline that excludes those consequences or measurements that identify them directly, and the sensitivity analysis reported above shows that the first is not sufficient on its own.
...
slightly shorter and, in aggregate, easier. Since the penalty is steeper at high effort
...
Hot sessions are shorter and easier & the penalty is steeper at high effort & \textbf{understates} \\
...
estimate of it. That is consistent with Section~\ref{sec:benchmark}, where forms fitted on actual
mass-participation races imply substantially steeper slopes than the one learned here. It is also why no
