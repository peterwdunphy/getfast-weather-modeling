...
Third, the per-runner result is narrower than it first appears. The score carries genuine individual information: matched to the correct runner it improves held-out prediction, and scrambling which runner receives which score removes the gain entirely. What cannot be established is that the information is heat sensitivity specifically. Were it so, the score's effect should grow as conditions warm, and it does not. It shifts predictions by roughly the same amount at every temperature, and only about half its value survives when it is constrained to act through the temperature curve. The remainder behaves like a general correction to that runner's predicted pace rather than a response to the weather.

The direct calibration compounds this by returning the wrong sign: runners the model marks as more heat-sensitive turn out to need their predictions adjusted in the opposite direction. Two explanations compete. The baseline already observes heart rate, so a runner who suffers in heat may have been slowed by the baseline on that evidence alone, leaving the personalized score to push back against a correction already made. Alternatively the deep model has the ordering genuinely wrong. Removing heart rate from the baseline eliminates the reversal, which favors the first explanation, but that estimate is too imprecise to settle the question. Distinguishing them requires a baseline in which it is clear what each feature stands in for. Until that exists, the score is usable as evidence that a particular runner tends to be over- or under-predicted, and not as a measurement of how much heat costs them.
