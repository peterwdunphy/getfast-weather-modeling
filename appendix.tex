...
$1\times10^{-4}$ over 1{,}000 steps, then follows a cosine decay. The reported model was trained for 1{,}000{,}000 steps and then resumed from its final state for a further 4{,}000{,}000 steps, with the decay stretched so that the rate reaches its floor of $5\times10^{-6}$ at step 5{,}000{,}000. Batch size 64, data-parallel across two GPUs. The architecture-search runs of Appendix~\ref{app:ablation} used 200{,}000 steps.

Checkpoints are written whenever held-out $R^2$ or error improves. In the reported run the two criteria
select different steps ($R^2$ peaks at 4.9 million, error at 4.33 million); the artifacts in this paper come
from the error-selected checkpoint.
...
grid, because above 28\,\textdegree C the corpus is too sparse to support per-runner estimates
(Appendix~\ref{app:ceiling}) and no held-out pace-model activity lies above the ceiling.
...
applied to the median shape at this point; as reported in Section~\ref{sec:results} it moves 6 of 37
grid points by at most 0.07 percentage points.

A single window's sensitivity is dominated by noise: within a runner, two windows with non-overlapping
histories correlate at about 0.2, so most of the observed spread in $s_a$ across windows is measurement
error rather than a difference between runners. Averaging the 15 most recent windows raises the agreement
between non-overlapping averages to about 0.65. The per-runner score is therefore the percentile rank of a runner's window-averaged
...
\section{Why Curves Stop at 28\,\textdegree C}
\label{app:ceiling}

Every curve in this paper is truncated at 28\,\textdegree C WBGT. The model returns values above that,
and the counterfactual sweep was run to 30, but the estimates there rest on too little data to report.

\begin{table}[!ht]
\centering\small
\caption{Observations available above each WBGT threshold, whole corpus before the ceiling is applied ($n=407{,}946$ activities with pace, WBGT and every baseline feature present).}
\label{tab:ceiling}
\begin{tabular}{@{}rrrr@{}}
\toprule
WBGT above & Runs & Share of corpus & Runners \\
\midrule
24\,\textdegree C & 44{,}466 & 10.900\% & 1{,}793 \\
26\,\textdegree C & 18{,}947 & 4.644\% & 1{,}302 \\
28\,\textdegree C & 4{,}205 & 1.031\% & 683 \\
30\,\textdegree C & 376 & 0.092\% & 160 \\
32\,\textdegree C & 19 & 0.005\% & 15 \\
\bottomrule
\end{tabular}
\end{table}

Support falls by a factor of about four between 26 and 28\,\textdegree C and by an order of magnitude or more per two degrees thereafter. At 28\,\textdegree C there
remain 4{,}205 runs from 683 runners, enough that a median across runners is anchored by more than a
handful of individuals. At 30 there are 376 runs from 160 runners, many contributing a single run. At
32 the quantity is meaningless.

The held-out evaluation binds tighter still: the modelling frame is itself capped at the ceiling, so no
held-out activity above 28\,\textdegree C is scored and nothing beyond that point could be checked
against a held-out outcome even in principle. One further
consideration points the same way. The flattening the model shows in its top few degrees is precisely where a sparse-data
artifact and a genuine physiological plateau are least distinguishable, so reporting it would invite an
interpretation the data cannot support.

Twenty-eight degrees is therefore the point where per-runner estimates still rest on hundreds of runners
rather than dozens. It is a reporting limit, not a claim that the response ends there; several published
forms continue to steepen well beyond it (Section~\ref{sec:benchmark}).
...
same held-out panel of 1{,}349 windows from 158 runners. Changes were applied one at a time where possible, since
bundling them makes attribution impossible; two exceptions are marked (the live date channel was bundled with the longer window, and the batch-size change ran together with the first working padding mask).
...
worse as well, indicating a harder optimization rather than an overfitting gap; over a third of the
runners (37.5\%) have fewer than 60 activities, so the additional slots are padding for them. Restoring the
